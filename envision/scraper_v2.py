...
GENOMICS_EXTS = {
    ...
    '.bed', '.gtf', '.gff', '.gff3', # Genomic annotations
    '.bigwig', '.bw', '.wig',       # Genomics tracks
    '.cel', '.idat',                # Microarray
    '.loom',                         # Single-cell
    '.mtx', '.mtx.gz',              # Sparse matrices (often single-cell)
}
...
DATA_PLATFORMS = [
    ...
    'physionet.org',
    'synapse.org',
    'aws.amazon.com/s3', 's3.amazonaws.com',
...
class ZipInspector:
    ...
    @staticmethod
    def inspect_via_range(download_url: str, session: requests.Session, 
                          max_tail_bytes: int = 65536) -> Optional[List[Dict]]:
        """
        ...
        for item in contents:
            if item.get("is_directory"):
                ...
                if "." in filename:
                    ...
                    for ext in sorted(EYE_IMAGING_EXTS | GENOMICS_EXTS | ARCHIVE_EXTS, 
...
def extract_weblinks_from_description(record: Dict) -> List[Dict]:
    ...
    for url in urls:
        ...
        skip_patterns = ['twitter.com', 'facebook.com', 'linkedin.com', 
...
class ZenodoScraper:
    ...
    def search(self, query: str, max_results: int = 1000, 
               ...
               inspect_zips: bool = True) -> List[Dict]:
        """
    ...
    def _should_keep(self, record: Dict) -> bool:
        ...
        if any(l.get('type') in ['data_platform', 'archive_download', 'direct_file'] 
               for l in weblinks):
            return True
