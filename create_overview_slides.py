...
add_bullet(tf1, "Backbone: Alibaba-NLP/gte-large-en-v1.5", size=16, bold=True)
add_bullet(tf1, "1024-dim embeddings, 8K token context", size=14, level=1, color=LIGHT_GRAY)
add_bullet(tf1, "", size=8)
...
add_bullet(tf2, "Only 452 curated examples needed", size=14, level=1, color=LIGHT_GRAY)
...
tf3 = set_text(box3, "Training Data (452 examples)", size=22, bold=True, color=ACCENT)
box3.text_frame.paragraphs[0].space_after = Pt(10)

add_bullet(tf3, "EYE_IMAGING: 99 examples", size=16, bold=True, color=SOFT_GREEN)
...
add_bullet(tf3, "EYE_SOFTWARE: 30 examples", size=16, bold=True, color=ACCENT)
add_bullet(tf3, "GitHub repos, model weights, toolboxes", size=13, level=1, color=LIGHT_GRAY)
add_bullet(tf3, "", size=6)
add_bullet(tf3, "EDGE_CASE: 90 examples", size=16, bold=True, color=ORANGE)
add_bullet(tf3, "Review papers, animal studies, non-imaging eye research", size=13, level=1, color=LIGHT_GRAY)
add_bullet(tf3, "", size=6)
add_bullet(tf3, "NEGATIVE: 233 examples", size=16, bold=True, color=RGBColor(0xEF, 0x53, 0x50))
